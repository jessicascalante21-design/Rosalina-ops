...
import { Download, Trash2, RefreshCw, ClipboardCopy, CheckCircle, LogOut, Users, ClipboardList, Hotel, TrendingUp, BarChart3, Sparkles, Bell, Mail, Building2 } from "lucide-react";
...
import { GuestRecord, getGuests } from "@/lib/guest-types";
...
export default function DailyReport() {
  ...
  const [roomStatuses, setRoomStatuses] = useState<Record<string, string>>(() => {
    try { return JSON.parse(localStorage.getItem("rosalina_room_status") || "{}"); }
    catch { return {}; }
  });
  ...
  const loadData = () => {
    try { setEntries(JSON.parse(localStorage.getItem("rosalina_report") || "[]")); } catch { setEntries([]); }
    setGuests(getGuests());
    try { setFaqEntries(JSON.parse(localStorage.getItem("rosalina_chat_faq") || "[]")); } catch { setFaqEntries([]); }
  };
  ...
  const todayEntries = useMemo(() => entries.filter((e) => isToday(e.timestamp) && e.type !== "pre-arrival"), [entries]);
  const preArrivalEntries = useMemo(() => entries.filter((e) => e.type === "pre-arrival"), [entries]);
  const serviceEntries = useMemo(() => todayEntries.filter((e) => e.type === "service"), [todayEntries]);
  const feedbackEntries = useMemo(() => todayEntries.filter((e) => e.type === "feedback"), [todayEntries]);
  const urgentCount = useMemo(() => serviceEntries.filter((e) => e.urgency === "Urgent").length, [serviceEntries]);
  const faqGroups = useMemo(() => groupFAQs(faqEntries), [faqEntries]);
  const todayFaqCount = useMemo(() => faqEntries.filter((e) => isToday(e.ts)).length, [faqEntries]);
  ...
  return (
    <div className="min-h-screen relative font-sans overflow-x-hidden">
      <PageHead title="PR Property Hub" description="Staff property management dashboard" />
      ...
      <div className="relative z-10 flex flex-col lg:flex-row min-h-screen">
        ...
        <div className="flex-1 min-w-0">
          <div className="p-5 lg:p-8 max-w-5xl">
            ...
            {/* TAB: PROPERTY HUB */}
            {activeTab === "property" && (
              <PropertyTab guests={guests} roomStatuses={roomStatuses} onSetRoomStatus={handleSetRoomStatus} />
            )}

            {/* TAB: GUESTS */}
            {activeTab === "guests" && (
              <GuestsTab guests={guests} onGuestsChange={setGuests} />
            )}
